...
Please write a C# program to make use of the Async Programming concepts and write the program so that all of the above happen in a correct order.*/
    internal class Event
    {
        ...
        {
            Task pickupCG =  pickUpChiefGuest();
            Task decorate =  decorateStage();
            Task foodMaterial =  dropRawFoodMaterial();

            Task reviewSpeech = Event.reviewSpeech();
            Task venueCheck = Event.venueCheck();
            Task bringPrizes = Event.bringPrizes();

            Task.WaitAll(foodMaterial);
            Task prepareFood = Event.prepareFood();

            Task.WaitAll(pickupCG, decorate, reviewSpeech, venueCheck);
            Task ChiefGuestSpeech = Event.ChiefGuestSpeech();

            Task.WaitAll(bringPrizes, ChiefGuestSpeech);
            Task distributePrizes = Event.distributePrizes();

            Task.WaitAll(prepareFood, ChiefGuestSpeech);
            Task distributeFood = Event.distributeFood();
